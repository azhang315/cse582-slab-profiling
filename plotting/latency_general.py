...
def process_kernel_data(data_dir, kernel_label, checkpoint_dir):
...
def main(type, data_dirs, output_dir):
    """Main function for processing and plotting data."""
    specific_sizes = [16, 32, 64, 128, 256, 512, 1024, 2048, 4096]  # Adjust these as needed
    latency_dfs = []

    for i, data_dir in enumerate(data_dirs, start=1):
        kernel_label = f"Kernel {i}"
        print(f"Processing {type} data for {kernel_label} from {data_dir}...")
        ...
        checkpoint_dir = f"checkpoints_{type}"
        kernel_data = process_kernel_data(data_dir, kernel_label, checkpoint_dir)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 latency_new.py <stress-ng_type> <raw_dir_kernel_1> <raw_dir_kernel_2> <output_dir>")
        sys.exit(1)

    type = sys.argv[1]
    data_dirs = sys.argv[2:-1]
    output_dir = sys.argv[-1]
    main(type, data_dirs, output_dir)
